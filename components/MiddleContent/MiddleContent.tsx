...
interface MiddleContentProps {
  fnFetchReddit?(...args: unknown[]): unknown;
  fnLoadMoreRedditItems?(...args: unknown[]): unknown;
  fnFetchSubReddit?(...args: unknown[]): unknown;
  fnLoadMoreSubRedditItems?(...args: unknown[]): unknown;
  fnExpandItem?(...args: unknown[]): unknown;
  fnSetModalItem?(...args: unknown[]): unknown;
  fnSetRouterPath?(...args: unknown[]): unknown;
}

class MiddleContent extends React.PureComponent<MiddleContentProps> {
  constructor(props) {
    super(props);
  ...
  onLoadMoreItems = () => {
    ...
    if (pageParams.subredditName) {
      this.props.fnLoadMoreSubRedditItems({
        ...params,
        sub: pageParams.subredditName,
      });
    } else {
  ...
  render() {
    ...
    return (
      <MiddleContentWrapper>
          ...
          {fetching || isEmpty(finalItems) ? (
            ...
            map(
              (name) => (
                <Card
                  key={name}
                  item={posts.model[name]}
                  onOpenModal={this.onOpenModal}
                  onCloseModal={this.onCloseModal}
                  onExpand={this.props.fnExpandItem}
                />
              ),
              finalItems
            )
        ...
        <PageActions>
          <Button width={174} onClick={this.onLoadMoreItems}>
            Load More
          </Button>
        </PageActions>
        <Footer>
...
const mapDispatchToProps = (dispatch) => ({
  ...
  fnLoadMoreRedditItems: (params) =>
    dispatch(contentActions.loadMoreRedditItems(params)),
  fnFetchSubReddit: (params) => dispatch(contentActions.fetchSubReddit(params)),
  fnLoadMoreSubRedditItems: (params) =>
    dispatch(contentActions.loadMoreSubRedditItems(params)),
