...
class DetailsCard extends PureComponent {
  ...
  render() {
    ...
    const itemComments = has(singleItem.name, this.props.comments)
      ? this.props.comments[singleItem.name]
      : [];
        ...
        <ContentArea borderNone width="100%">
          ...
          <ContentRow>
            <ContentDetails>
              ...
              <ContentURL
                href={`https://www.reddit.com${singleItem.permalink}`}
                target="_blank"
              >
                {singleItem.permalink}
              </ContentURL>
              <ExpandableContent>
...
const mapDispatchToProps = (dispatch) => ({
  fnFetchPostComments: (item) =>
    dispatch(contentActions.fetchPostComments(item)),
});
