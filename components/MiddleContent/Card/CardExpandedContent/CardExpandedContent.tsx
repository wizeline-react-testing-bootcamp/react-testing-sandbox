...
import { DetailContent, DetailImage } from "../Card.styles";
...
const expandedContent = ({ item, expanded }: expandedContentProps) => {
  // if (expanded) {
  if (item.selftext) {
    return (
      <DetailContent>
        <Markdown source={item.selftext} linkTarget="_blank" />
      </DetailContent>
    );
    ...
    let videoId;
    if (regexFilters.youtubeShort.test(item.url)) {
      //https://youtu.be/{id}
      ...
      return (
        <DetailContent>
          <video
            ...
            muted=""
          >
            <source src={videoFile} type="application/vnd.apple.mpegURL" />
          </video>
        </DetailContent>
      );
    }
